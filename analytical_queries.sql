...
shares AS (
	...
	ROUND(100.0 * digital_m / NULLIF(digital_m + traditional_m, 0), 2) AS digital_share_pct
    FROM pivoted
)
SELECT year, digital_m, traditional_m, digital_share_pct,
    ROUND(AVG(digital_share_pct) OVER (ORDER BY year ROWS BETWEEN 2 PRECEDING AND CURRENT ROW)::numeric, 2)
	AS digital_share_3yr_moving_avg
FROM shares
ORDER BY year;
